...
O console do navegador exibe uma mensagem parecida com:
...
Certifique-se de que fez o deploy recente (`npm run deploy` ou `npm run cf:deploy`) para que esses cabeçalhos estejam ativos na sua URL pública.
...
O comando `npm run deploy` falha dizendo que o banco D1 não foi localizado.
...
O comando `npm run dev` avisa que a porta `8787` já está ocupada por outro processo.
