/**
 * ============================================================================
 * LMS BNCC COMPUTAÇÃO - ROTAS DE CURSOS (COURSES)
 * ============================================================================
...
export async function atualizarCurso(request, env, db, cursoId) {
  ...
  if (payload.perfil === 'instrutor' && cursoExistente.instrutor_id !== payload.id) {
    throw new ErroHttp('Você não tem permissão para editar este curso.', 403);
  }
...
export async function alterarStatusCurso(request, env, db, cursoId) {
  ...
  if (payload.perfil === 'instrutor' && cursoExistente.instrutor_id !== payload.id) {
    throw new ErroHttp('Você não tem permissão para alterar o status deste curso.', 403);
  }
...
export async function excluirCurso(request, env, db, cursoId) {
  ...
  if (payload.perfil === 'instrutor' && cursoExistente.instrutor_id !== payload.id) {
    throw new ErroHttp('Você não tem permissão para excluir este curso.', 403);
  }
